...
// Terminal state of a card process (`tipo: "CartaoEntregue"`): the card is in the
// applicant's hands and nothing will follow it. We only persist the label, so match
// the two labels the API ships for that tipo, tolerating case and spacing drift.
const DELIVERED_RE = /^\s*(cart[ãa]o\s+entregue|card\s+delivered)\s*$/i;
...
function isCardDelivered(estado) {
  return typeof estado === 'string' && DELIVERED_RE.test(estado);
}
