  ...
  try {
    ...
    if (type === 'input') {
      ...
      if (inputElement.length > 0) {
        ...
        if (fieldName === 'Estado') {
          ...
          if (returnValue) {
            ...
            console.log(`Found ${fieldName} in input ${id}: "${result}"`);
            return result;
          }
        }
        console.log(`Found ${fieldName} in input ${id}: "${trimmed}"`);
        return trimmed;
      }
    } else if (type === 'span') {
      ...
      if (spanElement.length > 0) {
        ...
        if (value) {
          console.log(`Found ${fieldName} in span ${id}: "${value}"`);
          return value;
        }
      }
    }
...
module.exports = { formatUrlLabel, extractFieldValue };
