...
const { isLegacyPortalUrl, isTrackingPortalUrl, isAuthenticatedLinkUrl } = require('./validate-url');
...
function createChecker({ bot, dbRun, dbAll, httpsAgent, fetcher }) {
  ...
  async function checkSingleUrl(urlData) {
    try {
      ...
      if (isTrackingPortalUrl(urlData.url)) {
        const label = formatUrlLabel(urlData.url, urlData.nome);
        return autoRemoveAndNotify(urlData, 'tracking portal URL',
          `⚠️ Removed from monitoring — tracking portal URL:\n${label}\n\nThis URL is from the AIMA process/card tracking portal (contactenos.aima.gov.pt), a different system that can't be monitored. Please send the URL from the renewals portal (portal-renovacoes.aima.gov.pt), typically found in the QR code of your submission confirmation document.`);
      }
