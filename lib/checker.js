...
const {
  ...
  getTrackingToken,
} = require('./validate-url');
const { PORTAL_FIELDS, fieldsForUrl } = require('./fields');
const { buildTrackingApiUrl, extractTrackingState, isCardDelivered } = require('./tracking');

const APPROVED_RETENTION_MONTHS = 2;
const DELIVERED_RETENTION_MONTHS = 1;
...
function isStaleBy(updatedAt, months, now) {
...
function isApprovedAndStale(estado, updatedAt, now = new Date()) {
  return isPedidoDeferido(estado) && isStaleBy(updatedAt, APPROVED_RETENTION_MONTHS, now);
}

// Card delivered is the terminal tracking state, so a row sitting on it untouched is
// only costing us requests.
function isDeliveredAndStale(estado, updatedAt, now = new Date()) {
  return isCardDelivered(estado) && isStaleBy(updatedAt, DELIVERED_RETENTION_MONTHS, now);
}
...
function createChecker({ bot, dbRun, dbAll, httpsAgent, fetcher }) {
  ...
  async function fetchPortalState(url) {
    const response = await request(url);
    const $ = cheerio.load(response.data);

    const currentValues = {};
    for (const { key, scrapeAs } of PORTAL_FIELDS) {
      currentValues[key] = extractFieldValue($, scrapeAs);
    }

    return { nome: $('span#P72_NOME_DISPLAY').text().trim() || null, currentValues };
  }

  // Returns null when AIMA does not know the link (404 or `encontrado: false`).
  async function fetchTrackingState(token) {
    let response;
    try {
      response = await request(buildTrackingApiUrl(token), 'application/json');
    } catch (error) {
      if (error.response && error.response.status === 404) return null;
      throw error;
    }

    const state = extractTrackingState(response.data);
    if (!state) return null;

    return {
      nome: state.numero,
      currentValues: {
        ultima_atualizacao: state.atualizadoEm,
        situacao_at_ss: null,
        estado: state.estado,
      },
    };
  ...
  async function checkSingleUrl(urlData) {
    try {
      // Every auto-removal guard below reports the row as we already know it; the
      // checks past them relabel with the freshly fetched `nome` instead.
      ...
      if (isUnsupportedTrackingPortalUrl(urlData.url)) {
        return autoRemoveAndNotify(urlData, 'unsupported contact portal URL',
          `⚠️ Removed from monitoring — unsupported link:\n${label}\n\nOn contactenos.aima.gov.pt only process/card tracking pages can be monitored. Those look like:\nhttps://contactenos.aima.gov.pt/tracking/<id>`);
      }

      if (isAuthenticatedLinkUrl(urlData.url)) {
        return autoRemoveAndNotify(urlData, 'authenticated session URL',
          `⚠️ Removed from monitoring — session-only URL:\n${label}\n\nThis link was tied to a logged-in portal session and stops working once that session ends, so it can't be monitored. Please send the public URL from the QR code on your submission confirmation document instead.`);
      }
      ...
      if (isDeliveredAndStale(urlData.estado, urlData.updated_at)) {
        return autoRemoveAndNotify(urlData, 'delivered card',
          `✅ Delivered card auto-removed from monitoring:\n${label}\n\n📍 Estado: ${urlData.estado}\n📅 Atualizado em: ${urlData.ultima_atualizacao}\n\nThe card was delivered more than ${DELIVERED_RETENTION_MONTHS} month ago and this is the final state, so monitoring has been stopped. Enjoy! 🎉`);
      }
      ...
      const trackingToken = getTrackingToken(urlData.url);
      const FIELDS = fieldsForUrl(urlData.url);
      const extracted = trackingToken
        ? await fetchTrackingState(trackingToken)
        : await fetchPortalState(urlData.url);

      if (!extracted) {
        return notify(urlData.chat_id,
          `⚠️ Tracking link not found:\n${formatUrlLabel(urlData.url, urlData.nome)}\n\nAIMA no longer recognises this tracking link — it may have expired or been replaced. Please check it in your browser and send the current link.`);
      }

      const { nome, currentValues } = extracted;
